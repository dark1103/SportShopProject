...
namespace SportShopProject.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public ActionResult ClientCreate(Client client)
        {
            ...
            client.Mail = UserManager.GetEmail(User.Identity.GetUserId());

            try
            {
                using (SportShopDBEntity entity = new SportShopDBEntity())
                {
                    if (entity.Clients.Where(c => c.UserName == client.UserName).Count() > 0)
                    {
                        throw new Exception("База данных уже содержит профиль");
                    }
                    entity.Clients.Add(client);
                    entity.SaveChanges();
                }
                return View("SuccessMessage", "Регистрация прошла успешно" as object);
            }
            catch
            {
                return View("FailMessage", "Произошла ошибка, попробуйте ещё раз." as object);
            }
        }
        ...
        public ActionResult ClientEdit(Client client)
        {
            try
            {
                using (SportShopDBEntity entity = new SportShopDBEntity())
                {
                    Client originalClient = entity.Clients.FirstOrDefault(c => c.UserName == User.Identity.Name);
                    
                    originalClient.LastName = client.LastName;
                    originalClient.FirstName = client.FirstName;
                    originalClient.Phone = client.Phone;
                    originalClient.Gender = client.Gender;
                    originalClient.Age = client.Age;
                    
                    entity.SaveChanges();
                }
                return View("SuccessMessage", "Изменение профиля прошло успешно" as object);
            }
            catch
            {
                return View("FailMessage", "Произошла ошибка, попробуйте ещё раз");
            }
        }
        ...
        public async Task<ActionResult> Index(ManageMessageId? message)
        {
            ...
            using(SportShopDBEntity entity = new SportShopDBEntity())
            {
                haveProfile = entity.Clients.Where(c => c.UserName == User.Identity.Name).Count() > 0;
            }
